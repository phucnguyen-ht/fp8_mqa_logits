            ...
            for mtp in "${MTP_LIST[@]}"; do
                for kv_length in "${KV_LENGTHS[@]}"; do
                    for batch in ${batches_str}; do
                        ...
                        # Wait for other processes on this GPU, abort after 3 retries
                        MAX_WAIT=3
                        WAIT_INTERVAL=10
                        waited=0
                        while true; do
                            other_pids=$(rocm-smi --showpids 2>/dev/null \
                                | awk -v g="${gpu_id}" '/^[0-9]/ { if ($3 == g) print $1 }' \
                                | tr '\n' ' ')
                            [[ -z "${other_pids// /}" ]] && break
                            if [ "${waited}" -ge "${MAX_WAIT}" ]; then
                                echo "[gpu ${gpu_id}] still detected other process(es) after $((MAX_WAIT * WAIT_INTERVAL))s: ${other_pids}; aborting tune on this GPU"
                                exit 0
                            fi
                            echo "[gpu ${gpu_id}] detected other process(es) on GPU ${gpu_id}: ${other_pids}; waiting ${WAIT_INTERVAL}s (attempt $((waited+1))/${MAX_WAIT})..."
                            sleep "${WAIT_INTERVAL}"
                            waited=$((waited + 1))
                        done
