        ...
        def wrapper(*args, **kwargs):
            ...
            if testGraph:
                ...
                with torch.cuda.graph(graph):
                    data = run_iters_rotate(1, func, rotate_args)
                with tpf.profile(
                ...
                # Uncomment below 3 lines if you want to export torch profiler trace file
                # export_file_name = f"cudagraph-trace-{str(int(time.time() * 1000))}.json"
                # prof.export_chrome_trace(export_file_name)
                # logger.info(f'With cudagraph: Export tracing result to {export_file_name}')
                return data, avg
            with tpf.profile(
                activities=[tpf.ProfilerActivity.CPU, tpf.ProfilerActivity.CUDA],
                profile_memory=True,
...
def get_trace_perf(prof, num_iters):
    ...
    if int(os.environ.get("AITER_LOG_MORE", 0)):
        ...
        logger.info(f"{df}")
    # Uncomment 2 below lines if you want to print torch profiler report
